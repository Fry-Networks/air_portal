...
const submitUrl = `http://${process.env.API_HOST}:${process.env.API_PORT}/api/submitAwair`;
export async function submitAwairKey(
    token: string,
    deviceId: string,
    address: string
    ...
    try {
        await axios
            .post(submitUrl, { token, deviceId, address })
            .then((response) => {
                const data: {
                    message: string;
